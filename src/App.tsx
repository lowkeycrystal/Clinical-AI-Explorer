...
export default function App() {
  ...
  const scales = ['All', 'Enterprise', 'Small Clinic'];
  ...
  const filteredTools = aiTools.filter(tool => {
    ...
    const matchesScale = selectedScale === 'All' || tool.targetScale === selectedScale || tool.targetScale === 'Both';
    return matchesSearch && matchesCategory && matchesDomain && matchesScale;
  });
          ...
          <div className="flex flex-col gap-3 mb-6">
            <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
              <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-500 uppercase tracking-wider bg-slate-100 rounded-md shrink-0">
                <Filter className="w-3.5 h-3.5" /> Scale
              </div>
              {scales.map(scale => (
                <button
                  key={scale}
                ...
                >
                  {scale === 'Both' ? 'Enterprise & Small Clinics' : scale === 'All' ? 'All' : scale}
                </button>
              ))}
            </div>
              ...
              {filteredTools.map((tool) => (
                ...
                >
                  ...
                  <div className="flex-1 mt-1">
                    <p className="text-sm text-slate-600">{tool.problemSolved}</p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-slate-100">
                    ...
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-purple-50 text-purple-700 text-xs font-medium border border-purple-100">
                      <Users className="w-3 h-3" /> {tool.targetScale === 'Both' ? 'Enterprise & Small Clinic' : tool.targetScale}
                    </span>
        ...
        <div className="space-y-8">
          ...
          <DetailSection title="Users & Target">
            <div className="space-y-4">
              ...
              <div>
                 <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block flex items-center gap-1.5">
                   <Filter className="w-3.5 h-3.5" /> Target Scale
                 </span>
                 <div className="flex flex-wrap gap-1.5">
                   <span className="px-2.5 py-1 bg-purple-50 border border-purple-100 rounded text-xs text-purple-700 shadow-sm font-medium">
                     {tool.targetScale === 'Both' ? 'Enterprise & Small Clinics' : tool.targetScale}
                   </span>
                 </div>
              </div>
            </div>
